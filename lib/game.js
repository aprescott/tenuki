...
const Game = function(boardElement, boardSize) {
  ...
  this.checkTerritoryStartingAt = function(y, x) {
    ...
    let startingPoint = this.intersectionAt(y, x);
    ...
    while (pointsToCheck.length > 0) {
      ...
      if (checkedPoints.indexOf(nextPoint) > -1) {
        ...
        this.neighborsFor(nextPoint.y, nextPoint.x).forEach(neighbor => {
    ...
    const occupiedPoints = boundaryPoints;
    const nonOccupiedPoints = checkedPoints;
