...
const Game = function(boardElement, boardSize) {
  ...
  this.groupAt = function(y, x, accumulated) {
    ...
    const nonEmptyNeighbors = this.neighborsFor(point.y, point.x).filter(neighbor => !neighbor.isEmpty());
  ...
  this.checkTerritoryStartingAt = function(y, x) {
    const pointsWithBoundary = this.surroundedPointsWithBoundaryAt(y, x);

    const occupiedPoints = pointsWithBoundary.filter(checkedPoint => {
      return !this.isDeadAt(checkedPoint.y, checkedPoint.x) && !checkedPoint.isEmpty();
    });

    const nonOccupiedPoints = pointsWithBoundary.filter(checkedPoint => {
      return this.isDeadAt(checkedPoint.y, checkedPoint.x) || checkedPoint.isEmpty();
    });
